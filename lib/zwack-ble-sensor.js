...
const VahanaCrewService = require('./ftms/vahana-crew-service');
...
if ( containsVCS ) { servicesOfferedArray[servicesOfferedArray.length] = VahanaCrewService.VahanaCrewServiceUUID }
...
class ZwackBLE extends EventEmitter {
	...
	constructor(options) {
		...
		this.vcs = new VahanaCrewService();
...
// 			}
	...
	notifyVCS(event) {
		debugFTMS(`[${this.name} notifyVCS] ${JSON.stringify(event)}`);

		this.vcs.notify(event);
	}
