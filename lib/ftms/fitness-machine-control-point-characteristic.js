...
const simulator = require('../../example/simulator');
...
//           debugFTMS('>>>>>>>>>>> Target Power(W): ' + targetPower + '[' + util.inspect(data) + ']');
      ...
      case SetTargetResistance:
        debugFTMS('[' + FitnessMachineControlPoint + '][FitnessMachineControlPointCharacteristic] onWriteRequest: Set target resistance');
        if (this.hasControl) {
          let targetResistance = data.readUInt8(1);
          debugFTMS('[' + FitnessMachineControlPoint + '][FitnessMachineControlPointCharacteristic] onWriteRequest: Set target resistance: ' + targetResistance);
          let message = {
          ...
          global.simulator.resistanceLevel = targetResistance;

          // Notify all connected clients about the new values
          this.fmsc.notifySetTargetResistance(targetResistance);
        }
        else {
          debugFTMS('Error: no control');
          response = this.result(code, ControlNotPermitted);
      ...
      case StartOrResume:
        ...
        if (this.hasControl) {
          if (this.isStarted) {
            ...
            global.simulator.totalDistance = 0;
            global.simulator.startTime = new Date();
